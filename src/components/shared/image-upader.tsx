...
import { deleteS3Objects } from "@/server/actions/delete-from-aws";
...
type Action =
  ...
  | {
      type: "UPLOAD_SUCCESS";
      payload: { id: string; url: string; key: string };
    }
  ...
  switch (action.type) {
    ...
    case "UPLOAD_ERROR":
      return {
        files: state.files.map((f) =>
          f.id === action.payload.id
            ? { ...f, status: "error", error: action.payload.error }
            : f,
        ),
      };
    case "DELETE_START":
      return {
        files: state.files.map((f) =>
          f.id === action.payload.id ? { ...f, status: "deleting" } : f,
        ),
      };
    case "DELETE_SUCCESS":
      return {
    ...
    case "DELETE_ERROR":
      return {
        files: state.files.map((f) =>
          f.id === action.payload.id ? { ...f, status: "success" } : f,
        ),
      };
    case "SYNC_FROM_VALUE": {
      const existingUrls = new Set(
        state.files.filter((f) => f.status === "success").map((f) => f.url),
      );
      const newUploaded = action.payload.filter(
        (v) => !existingUrls.has(v.url),
      );
...
export function MultiImageUploader({
  ...
  const finalAcceptTypes = acceptTypes
    ? [...acceptTypes]
    : [...defaultAcceptedTypes];
  if (acceptVideos) {
    videoMimeTypes.forEach((vt) => {
      if (!finalAcceptTypes.includes(vt)) finalAcceptTypes.push(vt);
    });
  }
  ...
  async function addFiles(selectedFiles: FileList | File[]) {
    ...
    for (let i = 0; i < Math.min(fileArray.length, remainingSlots); i++) {
      ...
      if (error) {
        toast.add({ title: "Invalid file", description: error });
        continue;
      }
  ...
  async function uploadFile(id: string, file: File) {
    ...
    try {
      ...
      if (!signed.success || !signed.data)
        throw new Error(signed.error || "Failed to get signed URL");
  ...
  useEffect(() => {
    onUploadingChange?.(
      files.some((f) => f.status === "uploading" || f.status === "deleting"),
    );
                    ...
                    >
                      {activeFile.mediaType === "video" ? (
                        <VideoPreview
                          src={
                            activeFile.url ||
                            URL.createObjectURL(activeFile.file)
                          }
                          className="w-full h-full object-cover"
                        />
                      ) : (
